...
$stmtImg = $objDb->prepare('SELECT url FROM properties_images WHERE pid = :pid AND type = "gallery"');
$resultImg = $stmtImg->execute(array('pid' => $_GET['id']));
$stmtImg->setFetchMode(PDO::FETCH_ASSOC);

// Query gallery images2
$stmtImg2 = $objDb->prepare('SELECT url FROM properties_images WHERE pid = :pid AND type = "gallery"');
$resultImg2 = $stmtImg2->execute(array('pid' => $_GET['id']));
$stmtImg2->setFetchMode(PDO::FETCH_ASSOC);

// Query floorplan images
$stmtFloorplan = $objDb->prepare('SELECT url FROM properties_images WHERE pid = :pid AND type = "floorplan"');
$resultFloorplan = $stmtFloorplan->execute(array('pid' => $_GET['id']));
$stmtFloorplan->setFetchMode(PDO::FETCH_ASSOC);

// Query neighborhoods
$stmtNeigh = $objDb->prepare('SELECT * FROM properties_neighborhoods WHERE pid = :pid');
$resultNeigh = $stmtNeigh->execute(array('pid' => $_GET['id']));
$stmtNeigh->setFetchMode(PDO::FETCH_ASSOC);
?>
				...
				<div class="col-md-8 right-sec">
								...
								<div class="left-sec col-md-7">
									<ul class="main-info-li">
										...
										<li>
											...
											<div class="value"><?php echo $row['property_status']; ?></div>
										</li>
										<li>
											<div class="title">Property Type :</div>
											<div class="value"><?php echo $row['property_type']; ?></div>
										</li>
									</ul>
								</div>
					...
					<div class="information-box">
						...
						<div class="box-content">
							...
							<?php while($rowNeigh = $stmtNeigh->fetch()){ ?>
								<div class="neighborhood-box col-md-6">
									<div class="btn-group" role="group" aria-label="...">
										<button type="button" class="btn btn-default title"><?php echo $rowNeigh['place']; ?></button>
										<button type="button" class="btn btn-default value"><?php echo $rowNeigh['distance']; ?> min</button>
										<button type="button" class="btn btn-default icon">
											<?php switch($rowNeigh['distance_by']){
												case 1: ?>
					...
					<div class="information-box">
						...
						<div class="box-content">
							<ul class="image-main-box clearfix">
								<?php while($rowFloorplan = $stmtFloorplan->fetch()){ ?>
									
									<li class="item col-xs-6">
										<figure>
											<img src="dashboard/<?php echo $rowFloorplan['url']; ?>" alt="11"/>
											<a href="dashboard/<?php echo $rowFloorplan['url']; ?>" class="more-details">Enlarge</a>
										</figure>
									</li>
